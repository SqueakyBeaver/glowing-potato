...
class DB {
    ...
    public function __construct() {
        try {
            ...
            $statement = $this->conn->query("SELECT COUNT(*) FROM INFORMATION_SCHEMA.SCHEMATA WHERE SCHEMA_NAME = '***REMOVED***'");
            if (!(bool) $statement->fetchColumn()) {
                // Create the database if it doesn't already exist
                $sql = "CREATE DATABASE ***REMOVED***";
                $this->conn->exec($sql);
            }

            $this->conn->exec("USE ***REMOVED***");
